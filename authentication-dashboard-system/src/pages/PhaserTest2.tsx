...
// Mock map data - 50×50 phaser tiles with 1:1 mapping (1 notropolis tile = 1 phaser tile)
// Using 2560×2560 high-resolution sprites
...
// Generate 50×50 map with road grid and random terrain
function generateMockTiles(): Tile[] {
  ...
  for (let y = 0; y < 50; y++) {
    for (let x = 0; x < 50; x++) {
      ...
      const isHorizontalRoad = y % 5 === 0;
      const isVerticalRoad = x % 5 === 0;
...
// Initial buildings - place motels in a scattered pattern (avoid roads)
function generateInitialBuildings(): BuildingInstance[] {
  const buildings: BuildingInstance[] = [];
  let buildingId = 1;

  // Place motels avoiding roads (every 5th tile is road)
  for (let y = 1; y < 50; y += 5) {
    for (let x = 1; x < 50; x += 5) {
      buildings.push({
...
export function PhaserTest2() {
  ...
  const [buildingIdCounter, setBuildingIdCounter] = useState(100);
  ...
  return (
    <div className="w-screen h-screen bg-gray-900 relative">
      ...
      <div className="absolute top-4 left-4 bg-black/70 text-white p-4 rounded-lg text-sm font-mono">
        <div className="text-green-400 font-bold mb-2">HD Test (2560×2560 sprites)</div>
        <div>Map: 50×50 (1:1 mapping)</div>
