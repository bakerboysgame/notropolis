...
import { gridToScreen, getDepth } from '../utils/coordinatesTopDown';
...
export class CharacterSystem {
  ...
  spawnCharacter(characterType: 'banana' | 'apple' = 'apple'): boolean {
    ...
    // Use grid position for depth sorting in top-down view
    sprite.setDepth(getDepth(gridX, gridY, 0.2));
  ...
  private updateCharacter(char: Character, delta: number): void {
    ...
    // Update depth for correct draw order (based on grid Y position)
    char.sprite.setDepth(getDepth(char.gridX, char.gridY, 0.2));
